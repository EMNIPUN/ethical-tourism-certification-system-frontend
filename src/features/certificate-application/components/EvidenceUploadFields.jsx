  ...
  return (
    <div className='rounded-2xl border border-[var(--border-soft)] bg-[var(--surface-white)] p-6 shadow-[var(--shadow-soft)]'>
      <div className='flex items-start justify-between gap-4'>
        <div>
          <h3 className='text-base font-bold text-[var(--text-950)]'>{title}</h3>
          <p className='mt-1 text-sm font-medium text-[var(--text-700)]'>{description}</p>
        </div>
        <span className='inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--surface-soft)] text-[var(--brand-700)]'>
          <UploadCloud size={18} />
        </span>
      </div>

      <div className='mt-5 flex flex-wrap items-center gap-3'>
        <label className='inline-flex cursor-pointer items-center justify-center rounded-xl bg-[var(--brand-700)] px-4 py-2 text-sm font-semibold text-white transition hover:brightness-105'>
          <input
            type='file'
            name={name}
            accept={accept}
        ...
        <span className='text-xs font-semibold text-[var(--text-500)]'>Max 15MB each</span>
      </div>

      <div className='mt-4'>
        {hasFiles ? (
          <ul className='space-y-2 text-sm font-medium text-[var(--text-700)]'>
            {multiple
              ? files.map((file) => (
                  <li key={file.name} className='rounded-xl border border-[var(--border-soft)] bg-[var(--surface-soft)] px-3 py-2'>
                    {file.name}
                  </li>
                ))
              : (
                  <li className='rounded-xl border border-[var(--border-soft)] bg-[var(--surface-soft)] px-3 py-2'>
                    {files.name}
                  </li>
                )}
          </ul>
        ) : (
          <p className='text-sm font-medium text-[var(--text-500)]'>No files selected.</p>
        )}
      </div>
    </div>
  )
